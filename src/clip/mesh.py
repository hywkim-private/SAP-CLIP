...
import torch
import src.clip.utils as utils
from src.clip.utils import device
import copy
import numpy as np
import PIL

class Mesh():
    def __init__(self, verts, faces, verts_normals, faces_normals, color=torch.tensor([0.0,0.0,1.0])):
        self.vertices = verts
        self.faces = faces
        self.vertex_normals = torch.nn.functional.normalize(verts_normals.to(device).float())
        self.face_normals = torch.nn.functional.normalize(faces_normals.to(device).float())
